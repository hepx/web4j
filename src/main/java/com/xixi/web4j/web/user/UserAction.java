...
public class UserAction {
	...
	public String initModifyPwdForm(@ModelAttribute("loginUser")UserInfoBean loginUser){
		return "/user/modifyPwdForm";
	}
	
	@RequestMapping(value="modifyPwd",method=RequestMethod.POST)
	public String processModifyPwdForm(@Valid @ModelAttribute("loginUser")UserInfoBean loginUser,BindingResult result){
		UserInfoBean user=this.userInfoService.find(loginUser.getUserName(), loginUser.getPassword());
		if(user==null){
			result.rejectValue("password", "oldPwd.fail", "old password is fial");
		}
		if(result.hasErrors()){
			return "/user/modifyPwdForm";
		}else{
			this.userInfoService.modifyPwd(user.getUserId(), MD5Util.MD5(loginUser.getNewPwd()));
			return "main/home";
		}
	}
}
